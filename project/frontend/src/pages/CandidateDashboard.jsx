...
import { User, FileText, GraduationCap, BarChart3, Loader2, CheckCircle2, Mail, ShieldAlert, Award, Lock, Edit3 } from 'lucide-react';
...
export default function CandidateDashboard() {
  ...
  const fetchProfile = async () => {
    try {
      ...
      if (candidateInfo) {
        ...
        setBankAccount(candidateInfo.documents?.bank_account_number || '');
        setIfscCode(candidateInfo.documents?.ifsc_code || '');
        setBankHolderName(candidateInfo.documents?.full_name || '');
        if (candidateInfo.documents?.bank_account_number) {
          setBankSuccess(true);
        }
      }
    } catch (err) {
  ...
  const handleBankVerification = async (e) => {
    ...
    setBankLoading(true);
    try {
      const res = await db.verifyBank(bankAccount, ifscCode, bankHolderName);
      if (res.status === 'verified') {
        setBankSuccess(true);
        alert('Bank details verified successfully!');
      } else {
        alert('Verification incomplete. Issues: ' + (res.issues || []).join(', '));
      }
      fetchProfile();
    } catch (err) {
  ...
  // Setup database custom route fetch profile helper
  db.getCandidateProfile = async () => {
    const userStr = localStorage.getItem('user');
    const token = userStr ? JSON.parse(userStr).token : '';
    const res = await fetch('http://localhost:8000/candidates/me', {
          ...
          <div className="bg-slate-800 rounded-xl p-6 border border-slate-700 shadow-xl flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-lg font-semibold">Bank details & Cancelled Cheque Details</h2>
              </div>
              
              {bankSuccess ? (
                <div className="bg-emerald-950/50 border border-emerald-900/50 rounded-xl p-6 text-center">
                  <CheckCircle2 className="w-10 h-10 text-emerald-400 mx-auto mb-2" />
                  <p className="text-sm font-semibold text-emerald-200">Bank Verification Confirmed</p>
                  <p className="text-xs text-slate-500 mt-1">Validated Holder: {bankHolderName}</p>
                  <p className="text-xs text-slate-500">IFSC Code: {ifscCode}</p>
                </div>
              ) : (
                <form onSubmit={handleBankVerification} className="space-y-4">
                  <div>
                    <label className="block text-xs font-medium text-slate-400 mb-1">Account Holder Legal Name</label>
                    <input
                      type="text"
                  ...
                  <button
                    ...
                    className="w-full py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium text-xs transition-colors flex items-center justify-center gap-2"
                  >
                    {bankLoading && <Loader2 className="w-3.5 h-3.5 animate-spin" />}
                    Submit Bank Details
                  </button>
                </form>
              )}
