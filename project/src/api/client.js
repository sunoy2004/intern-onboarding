...
export const db = {
  ...
  async getDocuments(candidateId) {
    ...
    vRecs.forEach(v => {
      const output = parseOutput(v.verification_output);
      docs.push({
    ...
    if (docData) {
      if (docData.pan_number && !docs.some(d => d.doc_type === 'pan_card')) {
        docs.push({ doc_type: 'pan_card', original_filename: 'PAN Card Verified', status: normalizeStatus(docData.verification_status) });
      }
      if (docData.aadhaar_number && !docs.some(d => d.doc_type === 'aadhaar_card')) {
        docs.push({ doc_type: 'aadhaar_card', original_filename: 'Aadhaar Card Verified', status: normalizeStatus(docData.verification_status) });
      }
      if (docData.bank_account_number && !docs.some(d => d.doc_type === 'bank_passbook')) {
        docs.push({ doc_type: 'bank_passbook', original_filename: 'Bank Passbook Verified', status: normalizeStatus(docData.verification_status) });
      }
    }
  ...
  async getOnboardingTasks(candidateId) {
    const headers = await getHeaders();
    const response = await fetch(`${GATEWAY_URL}/hr/candidates/${candidateId}/session`, { headers });
    if (!response.ok) return [];
    const data = await response.json();
